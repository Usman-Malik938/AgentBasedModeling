...
def display_ai_response(image_path):
    ...
    with col1:
        st.header("🤖🧠 I am the Informational AI manager and have compiled the following information on this matter:")
        st.success(st.session_state.ai_response)


        st.write("Based on the information provided above, if you would like to resubmit your idea please enter it below.")
    ...
    with col2:
        ...
        display_media(image_path)

# Helper function to display media (image and video)
def display_media(image_path):
    image_width = 450
...
def display_resubmit_complete_buttons():
    ...
    col2 = st.columns(1)[0]  # Create a single column for the complete button
...
def display_practice_task(image_path):
    ...
    if not st.session_state.practice_submitted:
        ...
        # Display AI Manager Information, Image, and Video after submission
        ...
        with col1:
            ...
            # Resubmit idea input field with half width
            st.session_state.resubmit_idea = st.text_area("💡 Resubmit your idea:", key="resubmit_text",
                                height=100,
                                )
            ...
            with col2:
                if st.button("Submit", key="next_button"):
                    if st.session_state.user_idea.strip():
                        get_ai_response()  # This will set current_screen to "ai_response"
